...
**Description**: 1 interactive Claude Code session covering all 8 features sequentially. Detailed prompts with full specs.
...
```bash
ate-features exec scaffold 0a
```

- [ ] `data/transcripts/treatment-0a/` created
...
**CRITICAL:** After implementing **each** feature, save your patch by running `git diff > data/patches/treatment-0a/<FN>.patch` (replacing `<FN>` with the feature ID, e.g., `F1`, `F2`, etc.), then reset with `git checkout . && git clean -fd` before moving to the next feature. If you cannot produce a fix for a feature, save an empty patch and move on.


Remember: save each patch with `git diff > data/patches/treatment-0a/<FN>.patch` and reset with `git checkout . && git clean -fd` before starting the next feature. Start with F1.
````
...
git diff > data/patches/treatment-0a/<feature-id>.patch
...
```bash
git -C data/langgraph diff > data/patches/treatment-0a/remaining.patch
...
```bash
ate-features exec verify-patches 0a
...
Update `data/transcripts/treatment-0a/metadata.json` with actual timing and outcome data:
...
Record observations in `data/transcripts/treatment-0a/notes.md`. Use this format:

```markdown
# Notes: Treatment 0a — Control: Full Context
