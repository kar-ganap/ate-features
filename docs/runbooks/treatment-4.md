# Runbook: Treatment 4 — Player-Coach

**Treatment**: 4 (Player-Coach)
**Description**: 1 interactive Claude Code session with Agent Teams. Team size: 4x2. Delegate mode OFF (player-coach). Vague prompts (titles + subsystems only). Communication: neutral.
...
```bash
ate-features exec scaffold 4
```

- [ ] `data/transcripts/treatment-4/` created
...
- [ ] `data/patches/treatment-4/` exists
...
**CRITICAL:** After implementing **each** feature, save your patch by running `git diff > data/patches/treatment-4/<FN>.patch` (replacing `<FN>` with the feature ID, e.g., `F1`, `F2`, etc.), then reset with `git checkout . && git clean -fd` before moving to the next feature. If you cannot produce a fix for a feature, save an empty patch and move on.


Remember: save each patch with `git diff > data/patches/treatment-4/<FN>.patch` and reset with `git checkout . && git clean -fd` before starting the next feature. Start with F1.
````
...
git diff > data/patches/treatment-4/<feature-id>.patch
...
```bash
git -C data/langgraph diff > data/patches/treatment-4/remaining.patch
...
```bash
ate-features exec verify-patches 4
...
Update `data/transcripts/treatment-4/metadata.json` with actual timing and outcome data:
...
Record observations in `data/transcripts/treatment-4/notes.md`. Use this format:

```markdown
# Notes: Treatment 4 — Player-Coach
