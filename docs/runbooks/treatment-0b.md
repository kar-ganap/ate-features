...
# 4. Record start time
echo "Started: $(date -Iseconds)" >> data/transcripts/treatment-0b/notes.md
```

---

## Feature: F1 — Pandas DataFrame/Series msgpack serialization
...
After implementing this feature, save the patch and reset:
```
git diff > data/patches/treatment-0b/F1.patch
...
## Feature: F2 — Generic Pydantic v2 type round-trip in checkpoint serde
...
After implementing this feature, save the patch and reset:
```
git diff > data/patches/treatment-0b/F2.patch
git checkout . && git clean -fd
```
...
## Feature: F3 — StrEnum preservation in checkpoint serde
...
After implementing this feature, save the patch and reset:
```
git diff > data/patches/treatment-0b/F3.patch
git checkout . && git clean -fd
```
...
## Feature: F4 — Nested Enum deserialization fix
...
After implementing this feature, save the patch and reset:
```
git diff > data/patches/treatment-0b/F4.patch
git checkout . && git clean -fd
```
...
## Feature: F5 — Reducer metadata ordering dependency
...
After implementing this feature, save the patch and reset:
```
git diff > data/patches/treatment-0b/F5.patch
git checkout . && git clean -fd
```
...
## Feature: F6 — BinaryOperatorAggregate ignores default_factory
...
After implementing this feature, save the patch and reset:
```
git diff > data/patches/treatment-0b/F6.patch
git checkout . && git clean -fd
```
...
## Feature: F7 — Nested message detection in stream_mode=messages
...
After implementing this feature, save the patch and reset:
```
git diff > data/patches/treatment-0b/F7.patch
git checkout . && git clean -fd
```
...
## Feature: F8 — Input message dedup for nested structures
...
After implementing this feature, save the patch and reset:
```
git diff > data/patches/treatment-0b/F8.patch
git checkout . && git clean -fd
```
...
# 3. Record end time
echo "Ended: $(date -Iseconds)" >> data/transcripts/treatment-0b/notes.md
