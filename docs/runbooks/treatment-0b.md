...
This is a per-feature treatment — run 8 separate sessions, one per feature. See the sub-sections below.

---

### 2.F1 — Pandas DataFrame/Series msgpack serialization
...
- [ ] Confirmed: launched with plain `claude` (no Agent Teams env var)
...
**CRITICAL:** After implementing this feature, save your patch by running `git diff > data/patches/treatment-0b/F1.patch`, then reset with `git checkout . && git clean -fd`.


Remember: save your patch with `git diff > data/patches/treatment-0b/F1.patch` and reset with `git checkout . && git clean -fd` when done.
````

- [ ] Pasted opening prompt
- [ ] Agent began working on F1

---

### 2.F2 — Generic Pydantic v2 type round-trip in checkpoint serde
...
**CRITICAL:** After implementing this feature, save your patch by running `git diff > data/patches/treatment-0b/F2.patch`, then reset with `git checkout . && git clean -fd`.


Remember: save your patch with `git diff > data/patches/treatment-0b/F2.patch` and reset with `git checkout . && git clean -fd` when done.
````

- [ ] Pasted opening prompt
- [ ] Agent began working on F2

---

### 2.F3 — StrEnum preservation in checkpoint serde
...
**CRITICAL:** After implementing this feature, save your patch by running `git diff > data/patches/treatment-0b/F3.patch`, then reset with `git checkout . && git clean -fd`.


Remember: save your patch with `git diff > data/patches/treatment-0b/F3.patch` and reset with `git checkout . && git clean -fd` when done.
````

- [ ] Pasted opening prompt
- [ ] Agent began working on F3

---

### 2.F4 — Nested Enum deserialization fix
...
**CRITICAL:** After implementing this feature, save your patch by running `git diff > data/patches/treatment-0b/F4.patch`, then reset with `git checkout . && git clean -fd`.


Remember: save your patch with `git diff > data/patches/treatment-0b/F4.patch` and reset with `git checkout . && git clean -fd` when done.
````

- [ ] Pasted opening prompt
- [ ] Agent began working on F4

---

### 2.F5 — Reducer metadata ordering dependency
...
**CRITICAL:** After implementing this feature, save your patch by running `git diff > data/patches/treatment-0b/F5.patch`, then reset with `git checkout . && git clean -fd`.


Remember: save your patch with `git diff > data/patches/treatment-0b/F5.patch` and reset with `git checkout . && git clean -fd` when done.
````

- [ ] Pasted opening prompt
- [ ] Agent began working on F5

---

### 2.F6 — BinaryOperatorAggregate ignores default_factory
...
**CRITICAL:** After implementing this feature, save your patch by running `git diff > data/patches/treatment-0b/F6.patch`, then reset with `git checkout . && git clean -fd`.


Remember: save your patch with `git diff > data/patches/treatment-0b/F6.patch` and reset with `git checkout . && git clean -fd` when done.
````

- [ ] Pasted opening prompt
- [ ] Agent began working on F6

---

### 2.F7 — Nested message detection in stream_mode=messages
...
**CRITICAL:** After implementing this feature, save your patch by running `git diff > data/patches/treatment-0b/F7.patch`, then reset with `git checkout . && git clean -fd`.


Remember: save your patch with `git diff > data/patches/treatment-0b/F7.patch` and reset with `git checkout . && git clean -fd` when done.
````

- [ ] Pasted opening prompt
- [ ] Agent began working on F7

---

### 2.F8 — Input message dedup for nested structures
...
**CRITICAL:** After implementing this feature, save your patch by running `git diff > data/patches/treatment-0b/F8.patch`, then reset with `git checkout . && git clean -fd`.


Remember: save your patch with `git diff > data/patches/treatment-0b/F8.patch` and reset with `git checkout . && git clean -fd` when done.
````

- [ ] Pasted opening prompt
- [ ] Agent began working on F8
...
git diff > data/patches/treatment-0b/<feature-id>.patch
...
```bash
git -C data/langgraph diff > data/patches/treatment-0b/remaining.patch
