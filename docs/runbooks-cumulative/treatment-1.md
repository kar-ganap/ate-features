...
`git diff > data/patches/treatment-1/<FN>.patch`

When **all** features are complete, save the combined patch:
`git diff > data/patches/treatment-1/cumulative.patch`


Remember: after each agent finishes, save its snapshot with `git diff > data/patches/treatment-1/<FN>.patch`. When all features are done, save the combined patch with `git diff > data/patches/treatment-1/cumulative.patch`. Start with F1.
````
...
git diff --staged > data/patches/treatment-1/cumulative.patch
```
...
```bash
git -C data/langgraph diff > data/patches/treatment-1/cumulative.patch
```
