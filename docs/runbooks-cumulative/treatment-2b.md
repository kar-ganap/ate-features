...
`git diff > data/patches/treatment-2b/cumulative.patch`


Remember: after each agent finishes, save its snapshot with `git diff > data/patches/treatment-2b/<FN>.patch`. When all features are done, save the combined patch with `git diff > data/patches/treatment-2b/cumulative.patch`. Start with F1.
````
...
git diff --staged > data/patches/treatment-2b/cumulative.patch
```
...
```bash
git -C data/langgraph diff > data/patches/treatment-2b/cumulative.patch
```
