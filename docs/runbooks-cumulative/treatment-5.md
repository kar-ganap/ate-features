...
1. `git diff > data/patches/treatment-5/<FN>.patch`
2. `git add -A`
...
Remember: after each feature, snapshot with `git diff > data/patches/treatment-5/<FN>.patch` then `git add -A`. When all features are done, save the combined patch with `git diff --staged > data/patches/treatment-5/cumulative.patch`. Start with F1.
````
