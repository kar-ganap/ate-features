...
`git diff > data/patches/treatment-3/<FN>.patch`

When **all** features are complete, save the combined patch:
`git diff > data/patches/treatment-3/cumulative.patch`


Remember: after each agent finishes, save its snapshot with `git diff > data/patches/treatment-3/<FN>.patch`. When all features are done, save the combined patch with `git diff > data/patches/treatment-3/cumulative.patch`. Start with F1.
````
